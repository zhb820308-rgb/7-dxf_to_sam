	...
	int maxOutputEntities)
{
	...
	if (maxOutputEntities < 0)
	{
		QMessageBox::warning(
			nullptr,
			QStringLiteral("DXF Import - Expansion Limit"),
			QStringLiteral(
				"The drawing exceeded an internal block expansion safety limit.\n\n"
...
omuPrimitive* Example1PytModule::importDxf(omuArguments& args)
{
	...
	if (maxOutputEntities == 0 || maxOutputEntities < -1)
	{
		return failImport(logger, errorLogger, importId, pathText,
	...
	const bool unlimitedOutput = maxOutputEntities < 0;
	const std::size_t outputLimit = unlimitedOutput
