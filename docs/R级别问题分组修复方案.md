...
基准提交：`4b48230`
制定日期：2026-08-01
当前状态：G1、G2 已关闭；其余 8 项 Required 待处理
...
6. C-02 的 C++ 最终输出预算暂按用户决定延期，不阻断 G1；后续高规模测试仍应避免导入不可信大文件。
...
|    3 | G3 图层语义   | R-03             | 正确实现 INSERT 图层继承与过滤 | 忽略图层失效                 |
...
1. 在 `InsertInfo` 中保存 INSERT 图层。
2. 在块实体数据或展开上下文中保留原始图层。
3. 为展开函数增加继承图层参数或显式上下文对象。
4. 删除只在解析回调阶段过滤、导致无法表达继承的分支。
5. 日志同时记录原始图层和最终有效图层。
