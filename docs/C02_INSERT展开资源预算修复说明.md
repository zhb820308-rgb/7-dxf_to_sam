...
900,000。`expandInsertArray()` 还会在逐实体检查前执行批量 `reserve`，需要
增加安全容量计算和预算前置检查。完整关闭 C-02 需要在最终转换阶段消费统一
预算，并补充多层嵌套、最终输出和容量预分配测试。
...
Sketch/FE 最终输出和批量 `reserve` 尚未纳入统一预算，C-02 当前为部分修复，
不能标记为完全关闭。
