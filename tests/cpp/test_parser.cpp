...
#ifndef DXF_TEST_DATA_DIR
#error "DXF_TEST_DATA_DIR must be provided by CMake"
#endif
...
// 所有 DXF 只保存在 tests/fixtures/dxf。按职责拼出子目录，避免测试再依赖
// example/ 或另一套重复的 regression 数据。
static const QString DXF_DATA_DIR = QString::fromUtf8(DXF_TEST_DATA_DIR);
static const QString BASIC_ELEMENT_DIR = DXF_DATA_DIR + "/basic_element";
static const QString BLOCK_DIR = DXF_DATA_DIR + "/block";
static const QString REGRESSION_DIR = DXF_DATA_DIR + "/regression";
...
TEST(Parser, basic_element_files_exist) {
    EXPECT_TRUE(fileExists(BASIC_ELEMENT_DIR + "/mixed_lines_lwpolyline.dxf"));
    EXPECT_TRUE(fileExists(BASIC_ELEMENT_DIR + "/circle.dxf"));
    EXPECT_TRUE(fileExists(BASIC_ELEMENT_DIR + "/ellipse.dxf"));
    EXPECT_TRUE(fileExists(BASIC_ELEMENT_DIR + "/lwpolyline_closed_square.dxf"));
    EXPECT_TRUE(fileExists(BASIC_ELEMENT_DIR + "/arc_half_circle.dxf"));
    EXPECT_TRUE(fileExists(BASIC_ELEMENT_DIR + "/arc_partial.dxf"));
    EXPECT_TRUE(fileExists(BASIC_ELEMENT_DIR + "/lwpolyline_half_circle.dxf"));
    EXPECT_TRUE(fileExists(BASIC_ELEMENT_DIR + "/rounded_corner_arc_lines.dxf"));
    EXPECT_TRUE(fileExists(BASIC_ELEMENT_DIR + "/closed_lines_arc.dxf"));
}
...
TEST(Parser, nonexistent_file_returns_false) {
    ...
    EXPECT_FALSE(parser.parseFile(
        REGRESSION_DIR + "/nonexistent_file_12345.dxf", data));
    EXPECT_FALSE(data.isValid());
}
...
TEST(Parser, invalid_curve_tolerance_is_rejected_before_reading) {
    ...
    DxfParser parser;
    EXPECT_FALSE(parser.parseFile(
        REGRESSION_DIR + "/point_only.dxf", data,
...
TEST(Parser, line1_dxf_has_lines) {
    DxfData data;
    DxfParser parser;
    ASSERT_TRUE(parser.parseFile(BASIC_ELEMENT_DIR + "/mixed_lines_lwpolyline.dxf", data));
...
TEST(Parser, circle_dxf_has_circles) {
    DxfData data;
    DxfParser parser;
    ASSERT_TRUE(parser.parseFile(BASIC_ELEMENT_DIR + "/circle.dxf", data));
    EXPECT_TRUE(data.isValid());
...
TEST(Parser, square_dxf_has_entities) {
    DxfData data;
    DxfParser parser;
    ASSERT_TRUE(parser.parseFile(BASIC_ELEMENT_DIR + "/lwpolyline_closed_square.dxf", data));
    EXPECT_TRUE(data.isValid());
...
TEST(Parser, point_only_dxf_is_valid_input) {
    ...
    ASSERT_TRUE(parser.parseFile(REGRESSION_DIR + "/point_only.dxf", data));
    EXPECT_TRUE(data.isValid());
...
TEST(Parser, half_circle_up_dxf_has_arcs) {
    DxfData data;
    DxfParser parser;
    ASSERT_TRUE(parser.parseFile(BASIC_ELEMENT_DIR + "/arc_half_circle.dxf", data));
    EXPECT_TRUE(data.isValid());
...
TEST(Parser, part_circle_down_dxf_has_arcs) {
    DxfData data;
    DxfParser parser;
    ASSERT_TRUE(parser.parseFile(BASIC_ELEMENT_DIR + "/arc_partial.dxf", data));
    EXPECT_TRUE(data.isValid());
    EXPECT_GE(data.arcs().size(), 1u);
}
...
TEST(Parser, elipse_dxf_has_ellipses) {
    DxfData data;
    DxfParser parser;
    ASSERT_TRUE(parser.parseFile(BASIC_ELEMENT_DIR + "/ellipse.dxf", data));
    EXPECT_TRUE(data.isValid());
...
TEST(Parser, pline_half_circle_dxf_has_polylines) {
    DxfData data;
    DxfParser parser;
    ASSERT_TRUE(parser.parseFile(BASIC_ELEMENT_DIR + "/lwpolyline_half_circle.dxf", data));
    EXPECT_TRUE(data.isValid());
...
TEST(Parser, control_spline_is_classified) {
    DxfData data;
    DxfParser parser;
    ASSERT_TRUE(parser.parseFile(BASIC_ELEMENT_DIR + "/spline_standard.dxf", data));
...
TEST(Parser, fit_only_spline_is_classified) {
    DxfData data;
    DxfParser parser;
    ASSERT_TRUE(parser.parseFile(BASIC_ELEMENT_DIR + "/spline_fit_only.dxf", data));
    ASSERT_FALSE(data.splines().empty());
    EXPECT_EQ(data.entityStats().splineCount(), data.splines().size());
...
TEST(Parser, nonuniform_block_curves_keep_source_statistics) {
    DxfData data;
    DxfParser parser;
    ASSERT_TRUE(parser.parseFile(REGRESSION_DIR + "/block_stats_nonuniform.dxf", data));
...
TEST(Parser, oversized_insert_array_is_rejected_without_partial_output) {
    ...
    EXPECT_FALSE(parser.parseFile(
        REGRESSION_DIR + "/block_array_limit.dxf", data));
    EXPECT_FALSE(data.isValid());
    EXPECT_EQ(data.entityCount(), 0);
...
TEST(Parser, unicode_and_punctuation_paths_are_supported) {
    ...
    for (const QString& relativePath : relativePaths) {
        ...
        ASSERT_TRUE(QFile::copy(REGRESSION_DIR + "/point_only.dxf", destination))
            << destination.toStdString();
...
TEST(Parser, unicode_staging_copy_is_removed_after_use) {
    ...
    ASSERT_TRUE(QFile::copy(REGRESSION_DIR + "/point_only.dxf", sourcePath));
...
TEST(Parser, unlimited_output_keeps_insert_array_safety_limit) {
    ...
    EXPECT_FALSE(parser.parseFile(
        REGRESSION_DIR + "/block_array_limit.dxf", data, 0.05, {},
...
TEST(Parser, file_with_only_invalid_supported_entities_fails) {
    ...
    DxfParser parser;
    EXPECT_FALSE(parser.parseFile(
        REGRESSION_DIR + "/invalid_entities_only.dxf", data));
    EXPECT_FALSE(data.isValid());
    EXPECT_EQ(data.entityCount(), 0);
...
TEST(Parser, block_base_point_is_applied_to_exact_line_coordinates) {
    const QString path = BLOCK_DIR + "/minimal_block_insert.dxf";
...
TEST(Parser, rotated_insert_array_has_exact_grid_coordinates) {
    const QString path = REGRESSION_DIR + "/block_array_rotated.dxf";
    ASSERT_TRUE(fileExists(path)) << path.toStdString();
...
TEST(Parser, nested_nonuniform_rotation_uses_matrix_composition) {
    const QString path = REGRESSION_DIR + "/block_nested_nonuniform.dxf";
    ASSERT_TRUE(fileExists(path)) << path.toStdString();
...
TEST(Parser, rotated_block_updates_curve_directions) {
    const QString path = REGRESSION_DIR + "/block_rotated_curves.dxf";
    ASSERT_TRUE(fileExists(path)) << path.toStdString();
...
TEST(Parser, mirrored_block_reverses_curve_direction_and_preserves_insert_z) {
    const QString path = REGRESSION_DIR + "/block_mirrored_curves.dxf";
    ASSERT_TRUE(fileExists(path)) << path.toStdString();
...
TEST(Parser, ignored_insert_layer_suppresses_the_entire_block_reference) {
    const QString path = REGRESSION_DIR + "/block_layer_inheritance.dxf";
    ASSERT_TRUE(fileExists(path)) << path.toStdString();
...
TEST(Parser, circle_coner_dxf) {
    DxfData data;
    DxfParser parser;
    ASSERT_TRUE(parser.parseFile(BASIC_ELEMENT_DIR + "/rounded_corner_arc_lines.dxf", data));
    EXPECT_TRUE(data.isValid());
    EXPECT_GT(data.entityCount(), 0);
}
...
TEST(Parser, line_and_circle_closed_dxf) {
    DxfData data;
    DxfParser parser;
    ASSERT_TRUE(parser.parseFile(BASIC_ELEMENT_DIR + "/closed_lines_arc.dxf", data));
    EXPECT_TRUE(data.isValid());
    EXPECT_GT(data.entityCount(), 0);
}
...
TEST(Pipeline, representative_basic_files_parse_and_convert) {
    const char* filenames[] = {
        "/mixed_lines_lwpolyline.dxf",
        "/circle.dxf",
        "/ellipse.dxf",
        "/lwpolyline_closed_square.dxf",
        "/arc_half_circle.dxf",
        "/arc_partial.dxf",
        "/lwpolyline_half_circle.dxf",
        "/rounded_corner_arc_lines.dxf",
        "/closed_lines_arc.dxf",
    };
    const int numFiles = sizeof(filenames) / sizeof(filenames[0]);

    for (int i = 0; i < numFiles; ++i) {
        QString path = BASIC_ELEMENT_DIR + filenames[i];
